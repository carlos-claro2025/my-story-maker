window.Templates = {
  ...
  render(type) {
    ...
    cells.forEach((cellDef, index) => {
      ...
      swap.setAttribute('aria-label', 'Trocar a foto desta célula');
  ...
  /* Recomendação de modelo a partir da quantidade de fotos na biblioteca.
     Regra simples e previsível: mosaico para poucas fotos, grade para muitas. */
  suggestFor(count) {
    const n = Number(count) || 0;
    if (n >= 5) return { template: 'grid3', reason: n + ' fotos ficam melhores na grade 3×3' };
    if (n >= 2) return { template: 'mosaic', reason: n + ' fotos ficam melhores no mosaico' };
    if (n === 1) return { template: 'empty', reason: 'uma foto só: use a tela inteira como fundo' };
    return null;
  },

  /* Atualiza o aviso de sugestão no painel de templates */
  refreshSuggestion() {
    const box = document.getElementById('templateSuggestion');
    if (!box) return null;
    const count = window.State?.feedImages ? window.State.feedImages.size : 0;
    const suggestion = this.suggestFor(count);
    if (!suggestion || suggestion.template === this.current) {
      box.style.display = 'none';
      return null;
    }
    box.style.display = 'flex';
    box.dataset.template = suggestion.template;
    const text = document.getElementById('templateSuggestionText');
    if (text) text.textContent = 'Sugestão: ' + suggestion.reason;
    return suggestion;
  },

  applySuggestion() {
    const box = document.getElementById('templateSuggestion');
    const template = box && box.dataset.template;
    if (!template) return false;
    const card = document.querySelector('.template-card[data-template="' + template + '"]');

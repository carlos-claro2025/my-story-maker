window.Export = {
  /* Multiplicador de resolução usado na exportação (1x, 2x, 3x…) */
  scale: 2,

  setScale(value) {
    const n = Number(value);
    if (n >= 1 && n <= 4) this.scale = n;
    return this.scale;
  },
  ...
  /* Copia o story renderizado para a área de transferência como PNG */
  async copyToClipboard() {
    if (!navigator.clipboard || typeof window.ClipboardItem === 'undefined') {
      return this.notify('Copiar imagem não é suportado neste navegador', 'warning');
    }
    try {
      const blob = await this.toBlob('png', 1);
      await navigator.clipboard.write([new window.ClipboardItem({ 'image/png': blob })]);
      this.notify('Imagem copiada para a área de transferência', 'success');
    } catch (err) {
      console.error(err);
      this.notify('Não foi possível copiar a imagem', 'error');
    }
  },

  /* Compartilhamento nativo (Web Share API nível 2) quando disponível */
  async share() {
    let file;
    try {
      const blob = await this.toBlob('png', 1);
      file = new File([blob], 'story.png', { type: 'image/png' });
    } catch (err) {
      console.error(err);
      return this.notify('Exportação indisponível neste ambiente', 'error');
    }
    if (!navigator.canShare || !navigator.canShare({ files: [file] })) {
      return this.notify('Compartilhar não é suportado aqui — baixe o PNG', 'warning');
    }
    try {
      await navigator.share({ files: [file], title: 'Meu story' });
    } catch (err) {
  ...
  async saveToDisk(format, quality) {
    ...
    let canvas;
    try {
      canvas = await this.render(this.scale);
    } catch (err) {
      console.error(err);
      return this.notify('Exportação indisponível neste ambiente', 'error');
    }

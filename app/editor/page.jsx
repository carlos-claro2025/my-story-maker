...
export default function EditorPage() {
    ...
    const [selectedTemplate, setSelectedTemplate] = useState("vazio");
    function initCanvas() {
    ...
    function applyFilterToActive(filterName) {
        ...
        switch (filterName) {
            case "quente":
                active.filters.push(new filters.Sepia({ amount: 0.35 }), new filters.Contrast({ contrast: 0.1 }), new filters.Brightness({ brightness: 0.05 }));
                break;
            case "frio":
                active.filters.push(new filters.HueRotation({ rotation: 0.2 }), new filters.Saturation({ saturation: -0.15 }), new filters.Brightness({ brightness: 0.05 }));
                break;
            case "vintage":
                active.filters.push(new filters.Sepia({ amount: 0.5 }), new filters.Contrast({ contrast: 0.15 }), new filters.Brightness({ brightness: -0.05 }));
                break;
            default:
                break;
        }
    ...
    return (<div className="min-h-screen bg-gray-50 text-gray-900">
      <header className="sticky top-0 z-10 bg-white/80 backdrop-blur border-b border-black/5">
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={clearCanvas} className="px-3 py-2 rounded-lg border border-black/10 bg-white text-sm font-semibold">
              Limpar
            </button>
            <button onClick={() => alert("Exportar story em PNG/JPG/WebP.")} className="px-3 py-2 rounded-lg bg-black text-white text-sm font-semibold">
              Exportar story
            </button>
          </div>
        </div>
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="flex flex-col gap-6">
        ...
        ].map(([key, label, fn]) => (<button key={key} onClick={() => {
                setSelectedTemplate(key);
                fn();
            }} className={`text-left rounded-xl border border-black/5 bg-gray-50 px-3 py-2 text-sm font-semibold ${selectedTemplate === key ? "ring-2 ring-black" : ""}`}>
                  {label}
                </button>))}
            </div>
          </section>
          ...
          <section className="rounded-2xl bg-white shadow-sm border border-black/5 overflow-hidden">
            ...
            <div className="p-3 grid grid-cols-3 gap-2">
              {Array.from({ length: 9 }).map((_, i) => (<div key={i} className="aspect-square rounded-xl bg-gradient-to-br from-rose-100 to-sky-100"/>))}
            </div>
        ...
        <section className="rounded-2xl bg-white shadow-sm border border-black/5 overflow-hidden">
          ...
          <div className="p-4 flex flex-col items-center gap-4">
            ...
            ["frio", "Frio"],
            ["vintage", "Vintage"],
        ].map(([key, label]) => (<button key={key} onClick={() => applyFilterToActive(key)} className="px-3 py-2 rounded-full border border-black/10 bg-white text-sm font-semibold">
                    {label}
                  </button>))}
              </div>
            </div>
            ...
            <div className="w-full">
              ...
              <div className="flex flex-wrap gap-2">
                {["Título", "Legenda", "#hashtag"].map((text) => (<button key={text} onClick={() => addText(text)} className="px-3 py-2 rounded-full border border-black/10 bg-white text-sm font-semibold">
                    {text}
                  </button>))}
              </div>
            </div>
